...
public class CustomNamedParameterJdbcTemplate {
    ...
    public boolean actualizar(String sql, Object object) {
        int result = 0;
        MapSqlParameterSource paramSource = crearParametros(object);
        result = this.namedParameterJdbcTemplate.update(sql, paramSource);
        if (result > 0) {
            return true;
        } else {
            return false;
    ...
    public boolean actualizar(String sql, MapSqlParameterSource paramSource) {
        int result = 0;
        result = this.namedParameterJdbcTemplate.update(sql, paramSource);
        if (result > 0) {
            return true;
    ...
    public boolean actualizar(String sql, String string) {
        int result = 0;
        MapSqlParameterSource paramSource = crearParametros(string);
        result = this.namedParameterJdbcTemplate.update(sql, paramSource);
        if (result > 0) {
            return true;
        } else {
